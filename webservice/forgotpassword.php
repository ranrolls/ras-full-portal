...
if($mode == "setpassword"){  
	...
	if(mysql_query("UPDATE ".$prefix."users set password = '$password',activation='' where id = '$userid' ")){
		...
		$result['valid']='1';  // Invalid Token
		
			echo json_encode($result); 
		       //echo"1";// Invalid Token
	}else{
		  header('Content-Type: application/json');
		 $result['invalid']='0';  // Invalid Token
			echo json_encode($result); 
	} 
}
...
################ FOrgot Username #############
			...
			$sql_username = " SELECT id,username from ".$prefix."users where email = '".$email."' ";
			$rs_username  = mysql_query($sql_username);
			$num_rows     = mysql_num_rows($rs_username);
			if($num_rows > 0){	
...
<tr>    <td height="20" align="left" valign="top"> <img src="http://'.$serverurl.'/images/banner123.jpg" alt=" " class="img" border="0" align="left" style="display:block;width:100%"></td>    </tr>
     ...
     <tr><td   align="center" valign="top">
       ...
       <tr><td align="left" valign="top"><span style="font-family:Arial, Helvetica, sans-serif; font-size:14px; line-height:20px; color:#343434; font-weight:normal;">Dear <span style="color:#343434;">user,</span><br /><br /> Please find your username below. You can use the RAS app to login to your account.</span>
<br /><br />
...
';
			...
			//echo $msg;  die;
				...
				sendmail($email,$subject,$msg,"info@ras.org.sg");
...
############ END HERE ############################
         ...
         ############ Forgot Password Mail Request ########################
			...
			if($num_rows > 0){	
				...
				//$msg .= $upload_fullpath."registration?view=reset&layout=confirm";
				 $msg .= "Go to Mobile App and Reset Your Password";
